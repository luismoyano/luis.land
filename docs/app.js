...
(function() {
  ...
  /**
   * Generate SVG path starting from circle position
   */
  function generatePathFromCircle() {
    if (!originCircle || !timelineSvg || !colorfulPath || !pencilPath) return;

    const circleRect = originCircle.getBoundingClientRect();
    ...
    if (heroSubtitle) {
      subtitleStartY = heroSubtitle.getBoundingClientRect().top;
    }
    
    // Generate waypoints based on content positions
    generateWaypoints();
    
    // Set the SVG viewBox to match the viewport
    timelineSvg.setAttribute('viewBox', `0 0 ${viewportWidth} ${viewportHeight}`);
    
    // Generate path data from waypoints
    const pathData = generatePathData();
    
    // Set up colorful path (thick)
    colorfulPath.setAttribute('d', pathData);
    colorfulPath.setAttribute('stroke-width', circleRadius);
    colorfulPath.setAttribute('stroke', '#3b82f6');
    
    // Set up pencil path (thin)
    pencilPath.setAttribute('d', pathData);
    pencilPath.setAttribute('stroke-width', circleRadius * PENCIL_WIDTH_RATIO);
    pencilPath.setAttribute('stroke', PENCIL_COLOR);
    
    // Calculate and store path length (both paths have the same length)
    pathLength = colorfulPath.getTotalLength();
    ...
    if (firstStory) {
      ...
      // Get the point on the path where the eraser (transition) is located
      const eraserPoint = colorfulPath.getPointAtLength(pencilTransitionStart);
      
      // Position the scribble so its starting point aligns with the eraser position
      // The scribble path starts at (SCRIBBLE_START_X, SCRIBBLE_START_Y) in its local coordinates
      // After scaling, these become (SCRIBBLE_START_X * SCRIBBLE_SCALE, SCRIBBLE_START_Y * SCRIBBLE_SCALE)
      // We need to translate so that point lands on the eraser position
      const scribbleOffsetX = eraserPoint.x - (SCRIBBLE_START_X * SCRIBBLE_SCALE);
      const scribbleOffsetY = eraserPoint.y - (SCRIBBLE_START_Y * SCRIBBLE_SCALE);
      scribblePosition = { x: scribbleOffsetX, y: scribbleOffsetY };
      
      // Set up the scribble path
      if (scribblePath && scribbleContainer) {
        scribblePath.setAttribute('d', SCRIBBLE_PATH);
        scribblePath.setAttribute('stroke-width', circleRadius * PENCIL_WIDTH_RATIO);
        scribblePath.setAttribute('stroke', PENCIL_COLOR);
        
        // Position and scale the scribble container
        // We scale first, then translate to align the start point with the eraser
        scribbleContainer.setAttribute('transform', 
          `translate(${scribblePosition.x}, ${scribblePosition.y}) scale(${SCRIBBLE_SCALE})`
        );
        
        // Get scribble path length for animation
        scribblePathLength = scribblePath.getTotalLength();
        
        // Calculate the end point of the scribble in screen coordinates
        // The scribble's local end point is at getPointAtLength(scribblePathLength)
        const localEndPoint = scribblePath.getPointAtLength(scribblePathLength);
        // Transform to screen coordinates: translate + scale
        scribbleEndPoint = {
          x: scribblePosition.x + localEndPoint.x * SCRIBBLE_SCALE,
          y: scribblePosition.y + localEndPoint.y * SCRIBBLE_SCALE
        };
        
        // Set up scribble for draw animation (hidden initially)
        scribblePath.style.strokeDasharray = scribblePathLength;
  ...
  function updateTimeline() {
    ...
    if (scrollY < scrollThreshold) {
      ...
      // Subtitle: slides down and fades out as user scrolls into the page (reversible)
      if (heroSubtitle) {
        const travelDistance = viewportHeight - subtitleStartY - 50;
        heroSubtitle.style.transform = `translateY(${travelDistance * t}px)`;
        heroSubtitle.style.opacity = String(1 - t);
      }
    } else {
      currentLineEndY = targetLineEndY;

      // Past threshold: subtitle is fully gone
      if (heroSubtitle) {
    ...
    let shouldEnterScribble = false;
    let shouldEnterReverseScribble = false;
    ...
    if (notebookSection) {
      const notebookRect = notebookSection.getBoundingClientRect();
      // Activate when the top of notebook-section reaches the top of viewport
      // Can re-enter if scribble is not complete (progress < 1)
      shouldEnterScribble = !isProgrammaticScrolling && notebookRect.top <= 0 && scribbleProgress < 1;
      
      // Check if we should enter reverse scribble mode:
      // - User has completed the scribble (progress >= 1)
      // - User is scrolling UP
      // - The notebook section bottom is at or below the viewport bottom
      const inReverseZone = notebookRect.bottom >= viewportHeight;
      
      shouldEnterReverseScribble = !isProgrammaticScrolling &&
                                    scribbleProgress >= 1 && 
                                    isScrollingUp && 
                                    inReverseZone;
    }
    
    if (!isScribbling && shouldEnterScribble) {
      isScribbling = true;
      // Reset accumulator when entering scribble mode
      scribbleScrollAccumulator = 0;
      scribbleProgress = 0;
      lockScroll();
      
      // Show the first story text immediately when scribble starts
      if (firstStory) {
        const storyContent = firstStory.closest('.story-content');
        if (storyContent) {
          storyContent.classList.add('visible');
        }
        // Add highlight animation class to each line
        const storyLines = firstStory.querySelectorAll('.story-line');
        storyLines.forEach(line => line.classList.add('highlight-animated'));
      }
    }
    
    // Enter reverse scribble mode - allows user to undraw the scribble
    if (!isScribbling && shouldEnterReverseScribble) {
      isScribbling = true;
      isReverseScribbling = true; // Mark that we're in reverse mode
      // Set accumulator to full since scribble is complete
      scribbleScrollAccumulator = SCRIBBLE_SCROLL_AMOUNT;
      scribbleProgress = 1;
      lockScroll();
      
      // Make sure story content stays visible with full highlight
      if (firstStory) {
        const storyContent = firstStory.closest('.story-content');
        if (storyContent) {
          storyContent.classList.add('visible');
    ...
    // If we're in scribble mode, draw the scribble based on progress
    if (isScribbling) {
      // Keep the colorful path fully drawn to the end
      targetLength = pathLength;
      
      // Draw the scribble path based on progress
      if (scribblePath && scribblePathLength > 0) {
        const scribbleOffset = scribblePathLength * (1 - scribbleProgress);
        scribblePath.style.strokeDashoffset = scribbleOffset;
      }
      
      // Update highlight progress on each line (one line at a time)
      if (firstStory) {
        const storyLines = firstStory.querySelectorAll('.story-line');
        const lineCount = storyLines.length;
        
        storyLines.forEach((line, index) => {
          // Each line gets a portion of the total progress
          const lineStart = index / lineCount;
          const lineEnd = (index + 1) / lineCount;
          
          // Calculate progress for this specific line (0 to 1)
          let lineProgress = 0;
          if (scribbleProgress >= lineEnd) {
            lineProgress = 1; // Line fully highlighted
          } else if (scribbleProgress > lineStart) {
            // Line is being highlighted
      ...
      // Check if scribble is complete (scrolled all the way down)
      // Don't exit if we're in reverse mode (we need to allow undrawing)
      if (scribbleProgress >= 1 && !isReverseScribbling) {
        isScribbling = false;
        unlockScroll();
      }
    }
    ...
    // === SCRIBBLE VISIBILITY ===
    // Keep scribble fully drawn if we've completed it
    if (!isScribbling && scribbleProgress >= 1 && scribblePath) {
      scribblePath.style.strokeDashoffset = 0;
    }
    
    // === COLORFUL PATH ===
    // Draws from 0 to targetLength (the path ends at the eraser)
    const colorfulOffset = pathLength - targetLength;
    
    colorfulPath.style.strokeDasharray = pathLength;
    colorfulPath.style.strokeDashoffset = colorfulOffset;
    
    // Update color based on scroll progress
    const scrollProgress = Math.min(Math.max(scrollY / (document.documentElement.scrollHeight - viewportHeight), 0), 1);
  ...
  function animateInitialSegment() {
    ...
    function animate(currentTime) {
      ...
      // Animate subtitle position
      if (heroSubtitle) {
        const translateY = subtitleTravelDistance * eased;
